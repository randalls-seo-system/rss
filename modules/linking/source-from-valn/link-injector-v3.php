<?php
/**
 * link-injector-v3.php — Contextual link injection with streaming output.
 * Plan is loaded from base64 in $args[0] or from the $PLAN variable set before this file.
 */
set_time_limit(600);
global $wpdb;

// Plan must be set as $PLAN variable before including this file
// (done by the wrapper script)
if (!isset($PLAN) || !is_array($PLAN)) {
    echo "ERROR: No plan loaded\n";
    exit;
}

function is_restricted($content, $pos) {
    $zones = [
...
function try_inject($content, $target_url, $anchor_text) {
    $tn = rtrim($target_url, '/');
    foreach (['href="'.$target_url.'"', 'href="'.$tn.'"', 'href="'.$tn.'/"',
              'href="https://valoannetwork.com'.$target_url.'"',
              'href="https://valoannetwork.com'.$tn.'"'] as $c) {
    ...
    foreach ($anchors as $a) {
        foreach ($pm[0] as $i => $m) {
            ...
            if (is_restricted($content, $ps)) continue;
            if (preg_match('~<a\s~i', $pi)) continue;
            // Word-boundary match to prevent sub-word splits (e.g. "aus" inside "because")
