<?php
/**
 * Plugin Name: VALN AI Crawler Monitor
 * Description: Logs hits from AI crawlers (GPTBot, ClaudeBot,
 *              PerplexityBot, etc.) to track AI retrieval activity.
 * Version: 1.0.0
 * Author: VALN
 */
...
add_action( 'init', function () {
    $version = get_option( 'valn_ai_crawler_db_version', '0' );
    if ( $version !== '1.0' ) {
        valn_ai_crawler_create_table();
        update_option( 'valn_ai_crawler_db_version', '1.0', false );
    }
}, 5 );

function valn_ai_crawler_create_table() {
    global $wpdb;
...
add_action( 'muplugins_loaded', 'valn_ai_crawler_detect', 1 );

function valn_ai_crawler_detect() {
...
add_action( 'valn_ai_crawler_prune', function () {
    global $wpdb;
    $table = $wpdb->prefix . 'valn_ai_crawler_log';
...
add_action( 'init', function () {
    if ( ! wp_next_scheduled( 'valn_ai_crawler_prune' ) ) {
        wp_schedule_event( time(), 'daily', 'valn_ai_crawler_prune' );
    }
}, 20 );
...
add_action( 'wp_dashboard_setup', function () {
    wp_add_dashboard_widget(
        'valn_ai_crawler_widget',
        'AI Crawler Activity',
        'valn_ai_crawler_dashboard_render'
    );
} );

function valn_ai_crawler_dashboard_render() {
    global $wpdb;
    $table = $wpdb->prefix . 'valn_ai_crawler_log';
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
    WP_CLI::add_command( 'valn ai-crawler', 'VALN_AI_Crawler_CLI' );
}

class VALN_AI_Crawler_CLI {
     ...
     * ## EXAMPLES
     *
     *     wp valn ai-crawler stats
     *     wp valn ai-crawler stats --days=7
     *
     * @subcommand stats
     */
