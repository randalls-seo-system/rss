<?php
/**
 * Plugin Name: VALN AI Crawler Dashboard
 * Description: WP Admin page showing AI crawler activity from wp_valn_ai_crawler_log.
 * Version: 1.0.0
 * Requires PHP: 7.4
 */
...
add_action( 'admin_menu', function () {
	add_menu_page(
		...
		'manage_options',
		'valn-ai-crawlers',
		'valn_ai_crawlers_render',
		'dashicons-visibility',
		81
	);
} );

function valn_ai_crawlers_render() {
	global $wpdb;
	$table = $wpdb->prefix . 'valn_ai_crawler_log';
	...
	if ( isset( $_GET['refresh'] ) && wp_verify_nonce( $_GET['_wpnonce'] ?? '', 'valn_ai_refresh' ) ) {
		delete_transient( 'valn_ai_crawler_stats' );
	}

	$stats = get_transient( 'valn_ai_crawler_stats' );
	if ( false === $stats ) {
		$stats = valn_ai_crawlers_query( $wpdb, $table );
		set_transient( 'valn_ai_crawler_stats', $stats, 300 );
	}

	$refresh_url = wp_nonce_url( admin_url( 'admin.php?page=valn-ai-crawlers&refresh=1' ), 'valn_ai_refresh' );
	...
	valn_ai_crawlers_css();
	valn_ai_crawlers_cards( $stats );
	valn_ai_crawlers_two_col( $stats );
	valn_ai_crawlers_chart( $stats );
	valn_ai_crawlers_recent( $stats );

	echo '</div>';
}

function valn_ai_crawlers_query( $wpdb, $table ) {
...
function valn_ai_crawlers_cards( $stats ) {
	$cards = array(
...
function valn_ai_crawlers_two_col( $stats ) {
...
function valn_ai_crawlers_chart( $stats ) {
...
function valn_ai_crawlers_recent( $stats ) {
