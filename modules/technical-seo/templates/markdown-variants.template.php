<?php
/**
 * RSS Technical SEO Module — markdown-variants.php
 ...
 * - {{FORM_PAGE_SLUG}} — slug of form/application page to skip (empty = disabled)
 *
...
add_action( 'template_redirect', function () {
    ...
    // Skip form/application pages (no good markdown conversion)
    $skip_slug = '{{FORM_PAGE_SLUG}}';
    if ( $skip_slug !== '' && $post->post_name === $skip_slug ) {
        return;
    }
    ...
    $cache_key = '{{SITE_PREFIX}}_md_' . $post->ID;
    $content   = get_transient( $cache_key );

    if ( false === $content ) {
        $content = {{SITE_PREFIX}}_md_render( $post );
...
add_action( 'save_post', function ( $post_id ) {
    delete_transient( '{{SITE_PREFIX}}_md_' . $post_id );
}, 20 );

/* ---------- Render ---------- */

function {{SITE_PREFIX}}_md_render( $post ) {
    ...
    // Tool page fallback
    if ( function_exists( '{{SITE_PREFIX}}_llms_is_tool_page' ) && {{SITE_PREFIX}}_llms_is_tool_page( $post->ID ) ) {
    ...
    if ( preg_match( '/\[{{SITE_PREFIX}}_|\[gravityform/', $post->post_content ) ) {
    ...
    $out .= {{SITE_PREFIX}}_md_html_to_markdown( $html );
...
function {{SITE_PREFIX}}_md_html_to_markdown( $html ) {
    ...
    $html = preg_replace( '/\[{{SITE_PREFIX}}_[^\]]*\]/', '', $html );
