...
- **L22: 18.4.13 ATF document order — BLUF positioned after cards.** On job `20260820-130406-831a3c31`, the BLUF section appears at character position 3922, after the quick cards at position 598. Expected ATF order: eyebrow → BLUF → cards → FAQs → body. This is the single genuine hard-assertion failure. Cause: `phase_h` assembly concatenates parts in order `[header, jump_nav, atf_lede, qstats, rating_bars, cards, atf_faqs, hub_box, bluf, ...]` — the BLUF is listed AFTER cards in the parts array. The spec expects BLUF before cards.
