...
- **L19: H1 architecture mismatch between rl-page and sanitizer.** `_build_header_prelude` (assemble-article.py:1774) generates H1 inside rl-hero. `_postprocess_structure` (html_sanitizer.py:297-301) strips it: "WP renders title separately" — correct for VALN's Divi architecture where H1 lives in a separate et_pb_section, not in post_content. The rl-page format expects H1 inline, so the validation gate fails on missing H1. Not a defect until the VALN postprocessor's H1 handling is resolved — the postprocessor may need to extract and relocate the H1, not preserve it.
