...
- **L21: validator result discarded on non-zero exit.** `_run_tool` (assemble-article.py line 646) raises `RuntimeError` on non-zero exit code. The validator exits non-zero when any hard assertion fails — that is normal operation, not a crash. The `except` handler (lines 2476-2479) catches the RuntimeError and defaults to `hard_passed=0, hard_total=30`. The validator's actual JSON output (e.g. 43/44 passed, one named failure) is never read. Consequence: every validation failure reports as 0/30 regardless of what actually failed, the failing assertion name is lost, and the threshold check (`hard_passed < 25`) compares against a fabricated number. The first run's reported "3 failures" was this default, not a real count. Candidate fix: read the validator's stdout (JSON) even when exit code is non-zero, parse the actual scores, and only fall back to 0/30 if the JSON itself is unparseable.
